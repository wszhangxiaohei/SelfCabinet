...
public class OrderService {
    ...
    public Cabinet distribution(Order order)throws Exception {
        ...
        if (cupboardMapper.getSpareNum(order.getBar_id())!=0){
            ...
            throw new SelfCabinetException(HttpStatus.INTERNAL_SERVER_ERROR.value(), ResponseMessage.ERROR, ResponseMessage.ERROR_CABINET_DISTRIBUTION);
        }
        else{
            //没有空余座位
    ...
    public String cancelOrder(String order_id)throws Exception{
        ...
        if (cabinet.getUsed().equals(Cabinet.USED)){
            return ResponseMessage.FAILED_CANCEl_ORDER;
        }
            ...
            throw new SelfCabinetException(HttpStatus.INTERNAL_SERVER_ERROR.value(), ResponseMessage.ERROR, ResponseMessage.ERROR_CABINET_USED);
        }
